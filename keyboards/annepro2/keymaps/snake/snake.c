...
void update_bitmap_from_cell(RGB bitmap[53], uint8_t cell, RGB color) {
  if (cell <= 11) {
    bitmap[cell].r = color.r;
    bitmap[cell].g = color.g;
    bitmap[cell].b = color.b;
  }
  else if (cell > 11 && cell <= 23) {
    bitmap[cell+2].r = color.r;
    bitmap[cell+2].g = color.g;
    bitmap[cell+2].b = color.b;
  }
  else if (cell > 23 && cell <= 35) {
    bitmap[cell+4].r = color.r;
    bitmap[cell+4].g = color.g;
    bitmap[cell+4].b = color.b;
  }
  else if (cell > 35) {
    bitmap[cell+5].r = color.r;
    bitmap[cell+5].g = color.g;
    bitmap[cell+5].b = color.b;
  }
}

void render_to_bitmap(RGB bitmap[53]) {
  // snake
  uint8_t snake_head_idx = snake_status.snake_head_idx;
  for (uint8_t i = 0; i < snake_status.snake_length; i++) {
    uint8_t snake_idx = (snake_head_idx >= i) ? snake_head_idx - i : SNAKE_LENGTH_MAX - (i - snake_head_idx);
...
void snake_update(uint32_t delta_time, RGB bitmap[53]) {
